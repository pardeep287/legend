 ...
 * To manage Purchase CRUD operations
 *
 **/
...
class PurchaseOrder extends Model
{
    ...
    {
        return $query->where('purchase_order.financial_year_id', financialYearId());
    }
    ...
    {
        return $query->where('purchase_order.company_id', loggedInCompanyId());
    }
    ...
    public function validatePurchase($inputs, $id = null, $isApi = false , $isEdited = false)
    {
        ...
        if($id) {
            $rules['purchase_number'] = 'unique:purchase_order,purchase_number,' . $id . ',id,deleted_at,NULL,financial_year_id,' . financialYearId();
        } else {
            $rules['purchase_number'] = 'required|unique:purchase_order,purchase_number,NULL,id,deleted_at,NULL,financial_year_id,' . financialYearId();
        }
    ...
    public function store($inputs, $id = null)
    {
        ...
        if ($id) {
            ...
            if (isset($inputs['update_item'])) {
                (new PurchaseOrderItems)->where('id',  $inputs['item_id'])->update($items);
    ...
    public function getPurchases($search = null, $skip, $perPage)
    {
      ...
      return $this->leftJoin('account_master', 'account_master.id', '=', 'purchase_order.account_id')
            ->financialyear()
            ->company()
            ->whereRaw($filter)
    ...
    public function getPurchaseSummary($search)
    {
        ...
        if(count($search) > 0 && isset($search['month']) && $search['month'] != "") {
            return $this->company()
                ...
                ->leftJoin('purchase_items as pi', 'pi.purchase_id', '=', 'purchase_order.id')
                ->whereRaw($filter)
    ...
    {
        return $this->where('purchase_order.id', $id)
                ->company()
                ->first(['purchase_order.purchase_number']);
    }
    /**
     *
     * @return array
    ...
    public function getLocalPurchaseByTax($search = null)
    {
        if ($search){
            $fields = [
                'purchase_items.cgst',
                'purchase_items.sgst',
                \DB::raw(" SUM(purchase_items.cgst_amount) as cgst_total"),
                \DB::raw(" SUM(purchase_items.sgst_amount) as sgst_total"),
                \DB::raw(" SUM(purchase_items.total_price) as total_purchase")
            ];

            $filter = $this->getFilters($search);
            return $this->join('purchase_items', function($join){
                $join->on('purchase_order.id','=','purchase_items.purchase_id');
                $join->on('purchase_order.purchase_type','=', \DB::raw("1"));
            })
                ->where("financial_year_id", financialYearId())
                ->whereRaw($filter)
                ->company()
                ->groupBy('purchase_items.cgst','purchase_items.sgst')
                ->get($fields);
    ...
    public function getCenterPurchaseByTax($search = null)
    {
        if ($search){
            $fields = [
                'purchase_items.igst',
                \DB::raw(" SUM(purchase_items.igst_amount) as igst_total"),
                \DB::raw(" SUM(purchase_items.total_price) as total_purchase")
            ];
            ...
            return $this->join('purchase_items', function($join){
                $join->on('purchase_order.id','=','purchase_items.purchase_id');
                $join->on('purchase_order.purchase_type','=', \DB::raw("2"));
            })
                ->where("financial_year_id", financialYearId())
                ->whereRaw($filter)
                ->company()
                ->groupBy('purchase_items.igst')
                ->get($fields);
            /*$query = \DB::getQueryLog();
            dd(end($query));*/
        }
     ...
     * @return mixed
     */
    public function getPurchaseOrderService($search = [])
    {
        ...
        foreach($data as $detail) {
            $result[$detail->id]  = paddingLeft($detail->purchase_number);
        }
        return ['' => '-Select Purchase Order-'] + $result;
    }
}
