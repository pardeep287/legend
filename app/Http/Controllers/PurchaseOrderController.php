...
class PurchaseOrderController extends Controller
{
    ...
    public function create($id = null)
    {
        ...
        $purchaseNumber = (new PurchaseOrder)->getNewPONumber();
        $products = (new Product)->getProductsService();
        //dd($products);
        return view('purchase-order.create', compact('bank', 'products', 'purchaseNumber' ));
    }
    ...
    {
        $inputs = $request->all();
        $validator = (new PurchaseOrder)->validatePurchase($inputs);
        if ($validator->fails()) {
        ...
        try
        {
            ...
            //echo convertToLocal($inputs['purchase_date']);

            $id = (new PurchaseOrder)->store($inputs);
           ...
           (new PurchaseOrder)->purchaseInvoiceUpdate($update, $id);
            ...
            $lang = lang('messages.created', lang('purchase_order.purchase_order'));
    ...
    {
        $result = (new PurchaseOrder)->getOrderDetail($id);
        $t = $request->get('t');
        $a = $request->get('a');
        if (!$result) {
            abort(404);
        ...
        $unit = [];
        $bank = (new Bank)->getBankService();
        $items = (new PurchaseOrderItems)->getPurchasesItems(['purchase_order_id' => $id]);
        $products = (new Product)->getProductsService();
        $unit = (new Unit)->getUnitService();
        return view('purchase-order.edit', compact('result', 'items', 'products', 'bank', 'unit', 't', 'a'));
    }
    ...
    public function addMoreUpdateCommon(Request $request, $id) {
        $result = (new PurchaseOrder)->company()->find($id);
        if (!$result) {
            return redirect()->route('purchase-order.index')
                ->with('error', lang('messages.invalid_id', string_manip(lang('purchase_order.purchase'))));
        }
        $inputs = $request->all();

        $a = ($inputs['a'] == 1) ? 1 : 0;
        $validator = (new PurchaseOrder)->validatePurchase($inputs, $id);
        ...
        try {
           ...
           /* dd($inputs);*/
            (new PurchaseOrder)->store($inputs, $id, true);
            ...
            //\DB::commit();
            $route = route('purchase-order.index');
            $lang = lang('messages.updated', lang('purchase_order.purchase_order'));
            ...
            elseif (isset($inputs['addmore'])) {
                ...
                $route = route('purchase-order.edit', $param);
                $lang = lang('messages.itemadded', lang('purchase_order.purchase_order'));
            }
            return validationResponse(true, 201, $lang, $route);
        } catch (\Exception $exception) {
            //\DB::rollBack();
    ...
    public function editItem($id, $itemId = null)
    {
      ...
      $items = (new PurchaseOrderItems)->getPurchaseItem(['id' => $itemId]);
      if (!$items) {
      ...
      return redirect()->route('purchase-order.edit', ['id' => $id, 't' => 'edit'])
    ...
    public function updateItem($id, $inputs)
    {
        try {
            ...
            $oldPurchase = (new PurchaseOrder)->company()->find($id);
            $oldPurchaseItems = (new PurchaseOrderItems)->where('id', $itemId)
            ...
            //dd($itemArray);
            (new PurchaseOrderItems)->store($itemArray, $itemId);
            ...
            // dd($purchaseArray);
            (new PurchaseOrder)->purchaseInvoiceUpdate($purchaseArray, $id);
            ...
            //\DB::commit();
            $route = route('purchase-order.edit', ['id' => $id, 't' => 'edit']);
            $lang = lang('messages.updated', lang('purchase_order.purchase_order_item'));
            return validationResponse(true, 201, $lang, $route);
        }
        catch (\Exception $exception) {
            //\DB::rollBack();
    ...
    public function purchasePaginate(Request $request, $pageNumber = null)
    {
        ...
        if (isset($inputs['form-search']) && $inputs['form-search'] != '') {
            ...
            unset($inputs['_token']);
            $data = (new PurchaseOrder)->getPurchases($inputs, $start, $perPage);
            $total = (new PurchaseOrder)->totalPurchases($inputs);
            $total = $total->total;
        } else {
            $data = (new PurchaseOrder)->getPurchases($inputs, $start, $perPage);
            $total = (new PurchaseOrder)->totalPurchases($inputs);
        ...
        return view('purchase-order.load_data', compact('inputs', 'data', 'total', 'page', 'perPage'));
    }
    ...
    public function dropItem($id, $itemId)
    {
        ...
        try {
            //\DB::beginTransaction();
            $result = (new PurchaseOrder)->company()->where('id', $id)->first();
            if (!$result) {
                return json_encode(array(
                    'status' => 0,
                    'message' => lang('messages.invalid_id', string_manip(lang('purchase_order.purchase')))
                ));
            }
            $resultItem = (new PurchaseOrderItems)->where('id', $itemId)
                                            ->where('purchase_order_id', $id)
                                            ->first([
                                                'cgst_amount',
                                                'sgst_amount',
            ...
            if (count($resultItem) > 0)
            {
                ...
                //dd($result);
                (new PurchaseOrder)->purchaseInvoiceUpdate($update, $id);
                (new PurchaseOrderItems)->dropItem($itemId);
                ...
                return json_encode(array(
                    'status' => 1,
                    'message' => lang('messages.itemDeleted', lang('purchase_order.purchase_order'))
                ));
            }
            ...
            //\DB::rollBack();
            return json_encode(array( 'status' => 0, 'message' => lang('messages.invalid_id', string_manip(lang('purchase_order.purchase_order'))) ));
        }
    }
