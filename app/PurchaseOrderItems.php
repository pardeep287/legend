 ...
 * To manage Purchase Order Items CRUD operations
 *
 **/
...
class PurchaseOrderItems extends Model
{
    ...
    public function getPurchasesItems($search = null)
    {
        ...
        if (is_array($search) && count($search) > 0) {
            $f1 = (array_key_exists('purchase_order_id', $search) && $search['purchase_order_id'] != "") ? " AND (purchase_order_items.purchase_order_id = " .
                addslashes(trim($search['purchase_order_id'])) . ")" : "";

            $filter .= $f1;
            return $this->leftJoin('purchase_order', 'purchase_order.id', '=', 'purchase_order_items.purchase_order_id')
                ->leftJoin('product', 'product.id', '=', 'purchase_order_items.product_id')
    ...
    public function getPurchaseItem($search = null)
    {
        ...
        $filter = 1; // default filter if no search
        if (is_array($search) && count($search) > 0) {
            $f1 = (array_key_exists('id', $search) && $search['id'] != "") ? " AND (purchase_order_items.id = " .
    ...
    public function getPurchaseReportTaxWise($search)
    {
        ...
        if(count($search) > 0) {
             return $this->leftJoin('purchase_order','purchase_order.id','=','purchase_order_items.purchase_id')
                ->leftJoin('tax','tax.id','=','purchase_order_items.tax_id')
    ...
    public function getFilters($search = [])
    {
        ...
        if (is_array($search) && count($search) > 0)
        {
            $f1 = (array_key_exists('purchase_type', $search) && $search['purchase_type'] != "") ? " and purchase_order.purchase_type = " .
                addslashes(trim($search['purchase_type'])) : "";

            $f2 = (array_key_exists('tax_category', $search) && $search['tax_category'] != "") ? " AND purchase_order_items.tax_id = '" .
            ...
            if (array_key_exists('month', $search) && $search['month'] != "" && $search['report_type'] == '2') {
                $filter .= " and " . \DB::raw('DATE_FORMAT(purchase_date, "%m")') . " = '" . paddingLeft($search['month']) . "' and purchase_order.financial_year_id = '" . financialYearId() . "'";
            }
